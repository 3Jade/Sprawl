...
namespace sprawl
{
	namespace collections
	{
		template<
			typename ValueType,
			typename Accessor1,
			typename Accessor2 = NullAccessor,
			typename Accessor3 = NullAccessor>
		class HashMap
		{
		public:

			typedef ValueType value_type;
			typedef AccessorGroup<ValueType, Accessor1, Accessor2, Accessor3> mapped_type;
			...
			bool insert(ValueType const& val, T1 const& key1, T2 const& key2, T3 const& key3)
			{
				mapped_type* newItem = (mapped_type*)allocator::alloc();
				::new((void*)newItem) mapped_type(val, key1, key2, key3);
				...
				, m_bucketCount(other.m_bucketCount)
			{
				...
				other.m_table2 = nullptr;
				other.m_table3 = nullptr;
				other.m_size = 0;
		...
		private:
			...
			SPRAWL_INDEXED_CLEAR(3);

			template<int i>
			void rehash_()
			{
				return rehash_(Specialized<i>());
			}

			SPRAWL_INDEXED_REHASH(1);
			SPRAWL_INDEXED_REHASH(2);
			SPRAWL_INDEXED_REHASH(3);
			...
			bool exists_(mapped_type* item)
			{
				...
				if (get_<3>(item->m_accessor3.GetKey()))
				{
					return true;
				}
				return false;
			...
			SPRAWL_INDEX_GETTER(3);

			template<int i, typename T>
			void erase_(T const& key)
			{
				erase_(key, Specialized<i>());
			}
			...
			{ \
				...
				{ \
					...
					{ \
						...
						if (hashMatch->prev3) \
						{ \
							hashMatch->prev3->next3 = hashMatch->next3; \
						} \
						else \
						{ \
							m_table3[hashMatch->idx3] = hashMatch->next3; \
						} \
						if (hashMatch->next3) \
						{ \
							hashMatch->next3->prev3 = hashMatch->prev3; \
						} \
